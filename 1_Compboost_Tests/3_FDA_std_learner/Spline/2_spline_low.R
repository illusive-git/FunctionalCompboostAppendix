...
est_comp_sort = data.frame(fdb_model_direct = matrix(matrix(unlist(fdb_model$coef()), ncol = dim(extract(fdb_model)[[1]][[1]])[2]
                                   ,byrow = TRUE),ncol = 1),
...
plot(est_comp_sort$fdb, type = "b", ylim = c(min(est_comp_sort[,1:2]),max(est_comp_sort[,1:2])), col = "red", pch = 3,
  main = "Beta Parameters 100 iters", ylab = expression(paste(beta," value")))
...
plot(colMeans(fdb_predict_error), type = "b", col = "blue", pch = 3, main ="Residuals - 100 Iteration", ylab = "Mean Residual", xlab = "t")
