...
comp_test$addBaselearner(feature = "a",id = "P1",bl_factory = BaselearnerPolynomial, degree = 1, intercept = FALSE)
comp_test$train(iteration = 100) 

fdb_model = FDboost(formula = fun ~ bols(a, lambda = 0,intercept = FALSE),
  timeformula = ~bbs(t, lambda = 0, knots = 25, degree = 3, differences = 2), data = fdb_long, id = ~id,
  control = boost_control(mstop = 100), offset = "scalar")
fdb_model$coef()[[1]]
...
par(mfrow = c(1,1))
plot(est_comp$FDb_1, type = "b", pch = 5, ylim = c(min(est_comp[,1:2]),max(est_comp[,1:2])), col = "red", main = "Baselearner")
